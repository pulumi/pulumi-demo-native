...
This boilerplate creates a working Pulumi-owned provider named `demo-native`.
...
1. Set the following options:
   * Owner: pulumi 
   * Repository name: pulumi-demo-native (replace "demo-native" with the name of your provider)
   * Description: Pulumi provider for demo-native
   * Repository type: Public
1. Clone the generated repository.

From the templated repository:

1. Search-replace `demo-native` with the name of your desired provider.
...
1. A `provider/` folder containing the building and implementation logic
   1. `cmd/`
      1. `pulumi-gen-demo-native/` - generates language SDKs from the schema
      2. `pulumi-resource-demo-native/` - holds the package schema, injects the package version, and starts the gRPC server
   2. `pkg`
...
3. `sdk` - holds the generated code libraries created by `pulumi-gen-demo-native/main.go`
4. `examples` a folder of Pulumi programs to try locally and/or use in CI.
5. A `Makefile` and this `README`.

### Writing the schema

The [JSON schema file](https://www.pulumi.com/docs/guides/pulumi-packages/schema) is used by `pulumi-gen-demo` to create language-specific SDKs. 
...
This repository provides the [demo example schema](./provider/cmd/pulumi-resource-demo/schema.json) to get you started.  
